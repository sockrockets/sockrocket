...
impl CommonState {
    ...
    //
    // The 5-byte record header is encoded into its own small chunk and the
    // payload `Vec` is appended by move, so queueing a record does not
    // allocate a second full-size buffer or copy the payload again
    // (as `OpaqueMessage::encode` would).
    fn queue_tls_message(&mut self, m: OpaqueMessage) {
        let (typ, version, payload) = m.into_header_and_payload();
        let mut header = Vec::with_capacity(OpaqueMessage::HEADER_SIZE as usize);
        typ.encode(&mut header);
        version.encode(&mut header);
        (payload.len() as u16).encode(&mut header);
        self.sendable_tls.append(header);
        self.sendable_tls.append(payload);
    }
