...
impl OpaqueMessage {
    ...
    /// Consume the message, returning the record header fields and the payload
    /// `Vec` moved out without copying.
    ///
    /// The send path uses this to queue the record as two chunks (a small
    /// header chunk plus the payload chunk), avoiding the extra allocation and
    /// full-payload copy that [`OpaqueMessage::encode`] performs.
    pub(crate) fn into_header_and_payload(self) -> (ContentType, ProtocolVersion, Vec<u8>) {
        (self.typ, self.version, self.payload.0)
    }
