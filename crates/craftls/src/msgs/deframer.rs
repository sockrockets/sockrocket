...
impl MessageDeframer {
    ...
    fn prepare_read(&mut self) -> Result<(), &'static str> {
        ...
        // payload is bounded by `OpaqueMessage::MAX_WIRE_SIZE` (`joining_hs` is only set after
        // the first record is parsed), the next time we come around here we allow a
        ...
        // If we can and need to increase the buffer size to allow a maximum-size
        // record read, do so. After
...
/// Read at most one full-size TLS record per socket read.  This avoids
/// splitting a maximum-size record over several read syscalls.
const READ_SIZE: usize = OpaqueMessage::MAX_WIRE_SIZE;
...
mod tests {
    ...
    fn test_limited_buffer() {
        ...
        // With READ_SIZE == OpaqueMessage::MAX_WIRE_SIZE, a whole maximum-size
        // record is taken in one read; the buffer then has room for
        // `MAX_WIRE_SIZE - message.len()` more bytes before refusing further data.
        let mut d = MessageDeframer::default();
        assert_len(message.len(), input_bytes(&mut d, &message));
        assert_len(
            OpaqueMessage::MAX_WIRE_SIZE - message.len(),
