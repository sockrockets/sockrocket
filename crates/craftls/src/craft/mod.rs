...
pub(crate) struct CraftConnectionData {
    ...
    /// REALITY auth key derived during the ClientHello, used to verify
    /// the server certificate. !craft!
    pub(crate) reality_auth_key: Option<Arc<[u8; 32]>>,
}
    ...
    fn to_rustls_extension(
        ...
        Ok(match self {
                    ...
                    versions
                        ...
                        .filter(|v| {
                            if !v.craft_is_unknown() && !origin_versions.contains(v) {
                                // Never panic a proxy connection on a fingerprint
                                // listing TLS versions rustls cannot emit; drop
                                // the unmatchable version instead.
                                debug_assert!(
                                    !craft_config.strict_mode,
                                    "unknown version {:?}, all: {:?}",
                                    v, origin_versions
            ...
            Self::Protocols(protocols) => {
                // Only enforce ALPN consistency when the fingerprint actually
                // overrides ALPN; with `do_not_override_alpn` the config's own
                // list is authoritative (e.g. http/1.1-only for WebSocket).
                if craft_config.strict_mode
                    && craft_config.override_alpn
...
impl FingerprintBuilder {
    ...
    /// Disables strict-mode fingerprint consistency asserts, keeping all
    /// overrides active. Needed for fingerprints that advertise TLS features
    /// this rustls base cannot emit (e.g. SAFARI_17_1 lists TLS 1.0/1.1 in
    /// supported_versions); without this a proxied connection panics.
    pub fn dangerous_disable_strict_mode(mut self) -> Self {
