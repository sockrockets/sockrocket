...
fn emit_client_hello_for_retry(
    ...
    // !craft! REALITY dual key share: emit both the hybrid (first provider
    // group) and classic (second provider group) shares. New xray servers
    // require the hybrid share to precede the classic one; old servers only
    // understand the classic one. The classic exchange is stashed in
    // `our_key_share_alt` so handle_server_hello can complete with whichever
    // group the server selects. Select by NAME (and require a retained
    // secret): fingerprint patching may have appended todo!() fake groups
    // to the provider list, so an index-based lookup can hit those.
    let mut reality_alt_share: Option<Box<dyn ActiveKeyExchange>> = None;
    if retryreq.is_none() && config.reality.is_some() {
        if let Some(primary) = &key_share {
            let want = match primary.group() {
                crate::msgs::enums::NamedGroup::X25519MLKEM768 => {
                    Some(crate::msgs::enums::NamedGroup::X25519)
                }
                _ => None,
            };
            if let Some(want) = want {
                for group in config.provider.kx_groups.iter() {
                    if group.name() != want {
                        continue;
                    }
                    if let Ok(kx) = group.start() {
                        if kx.secret_key().is_some() {
                            reality_alt_share = Some(kx);
                            break;
                        }
                    }
                }
    ...
    if let Some(key_share) = &key_share {
        ...
        if let Some(alt) = &reality_alt_share {
            shares.push(KeyShareEntry::new(alt.group(), alt.pub_key()));
        }
    ...
    // !craft! begin
    // REALITY: replace the session id with the encrypted auth token.
    // Auth uses the CLASSIC share's secret when both shares were offered —
    // xray's server-side auth loop prefers the standalone X25519 share over
    // the hybrid's X25519 component whenever both are present.
    if retryreq.is_none() {
        if let Some(reality) = &config.reality {
            crate::reality::patch_client_hello(
                &mut ch,
                &input.random,
                reality,
                reality_alt_share.as_deref().or(key_share.as_deref()),
                &mut cx.data.craft_connection_data.reality_auth_key,
            )?;
        }
        // shadow-tls v3: sign the session id with HMAC-SHA1(password).
        if let Some(shadow_tls) = &config.shadow_tls {
            crate::shadow_tls::patch_client_hello(&mut ch, shadow_tls)?;
        }
    }
    // Stash the alternate (classic) share so the ServerHello handling can
    // complete the key exchange with whichever group the server selected.
    if let Some(alt) = reality_alt_share {
        cx.data.craft_connection_data.our_key_share_alt.push(alt);
    }
    // !craft! end
...
impl State<ClientConnectionData> for ExpectServerHelloOrHelloRetryRequest {
    fn handle(self: Box<Self>, cx: &mut ClientContext<'_>, m: Message) -> NextStateOrError {
        // !craft! begin
        // REALITY cannot survive a HelloRetryRequest (the encrypted session
        // id is bound to the first ClientHello's encoding).
        if self.next.input.config.reality.is_some()
            && matches!(
                m.payload,
            ...
            )
        {
            return Err(Error::General(
                "REALITY: HelloRetryRequest is not supported".into(),
            ));
        }
        // !craft! end
        match m.payload {
