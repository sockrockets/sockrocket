...
impl State<ClientConnectionData> for ExpectCertificateVerify {
    fn handle(mut self: Box<Self>, cx: &mut ClientContext<'_>, m: Message) -> hs::NextStateOrError {
        ...
        // !craft! begin
        // REALITY: verify the Ed25519 self-signed certificate against the
        // auth key derived from the ClientHello instead of the configured
        // verifier.
        let reality_auth_key = cx.data.craft_connection_data.reality_auth_key.clone();
        let cert_verified = if let Some(ref auth_key) = reality_auth_key {
            match crate::reality::verify_end_entity(end_entity, &auth_key) {
                Ok(()) => verify::ServerCertVerified::assertion(),
                Err(err) => {
                    return Err(cx.common.send_cert_verify_error_alert(err));
                }
            }
        } else {
            self.config
        ...
        let sig_verified = if reality_auth_key.is_some() {
            // !craft! REALITY: verify the CertificateVerify signature with
            // the certificate's Ed25519 key.
            match crate::reality::verify_cert_verify_signature(
