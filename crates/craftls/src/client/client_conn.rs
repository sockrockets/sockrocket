...
pub struct ClientConfig {
    ...
    /// !craft! REALITY protocol settings. When set, the ClientHello session
    /// id carries the REALITY auth token and the server certificate is
    /// verified with the REALITY HMAC scheme instead of the configured
    /// verifier.
    pub reality: Option<Arc<crate::reality::RealityConfig>>,

    /// !craft! shadow-tls v3 settings. When set, the ClientHello session id
    /// carries the HMAC signature the shadow-tls server uses to authenticate
    /// the client before switching to the proxied protocol.
    pub shadow_tls: Option<Arc<crate::shadow_tls::ShadowTlsConfig>>,
}
...
impl Clone for ClientConfig {
    fn clone(&self) -> Self {
        Self {
            ...
            reality: self.reality.clone(),
            shadow_tls: self.shadow_tls.clone(),
        }
    }
}
...
pub struct Resumption {
    ...
    pub store: Arc<dyn ClientSessionStore>,
