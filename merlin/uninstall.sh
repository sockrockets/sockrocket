...
# ── Stop everything + restore direct LAN network FIRST ────────────────────────
# Must run while scripts still exist. Softcenter uninstall leaving a DNS pin
# to 127.0.0.1#5300 is the most common "network dead after uninstall" failure.
info "Stopping services and restoring direct network..."
[ -x "$SOCKROCKET_DIR/scripts/sockrocket.sh" ] && {
    "$SOCKROCKET_DIR/scripts/sockrocket.sh" stop 2>/dev/null || true
    "$SOCKROCKET_DIR/scripts/sockrocket.sh" network-restore 2>/dev/null || true
...
[ -x "$SOCKROCKET_DIR/scripts/iptables.sh" ] && {
    ...
    "$SOCKROCKET_DIR/scripts/iptables.sh" dns-stop 2>/dev/null || true
}

# Inline fail-open if scripts already missing / stop failed midway
...
# Final network sanity: no DNS pin / no Sockrocket iptables after uninstall
if [ -f /tmp/resolv.dnsmasq ] && grep -q "^server=127.0.0.1#" /tmp/resolv.dnsmasq 2>/dev/null; then
    warn "DNS still pinned to loopback — forcing ISP restore"
    awk '/^nameserver/ && $2 != "127.0.0.1" && $2 != "::1" {print "server=" $2}' \
        /etc/resolv.conf 2>/dev/null > /tmp/resolv.dnsmasq.new \
        && [ -s /tmp/resolv.dnsmasq.new ] \
...
if iptables -t mangle -L SOCKROCKET_MANGLE -n 2>/dev/null | grep -q MARK; then
    warn "SOCKROCKET_MANGLE still present — force flush"
    iptables -t mangle -D PREROUTING -j SOCKROCKET_MANGLE 2>/dev/null || true
    iptables -t mangle -F SOCKROCKET_MANGLE 2>/dev/null || true
    iptables -t mangle -X SOCKROCKET_MANGLE 2>/dev/null || true
    leftover=$((leftover + 1))
