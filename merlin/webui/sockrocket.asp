...
function api(action, data, cb, errCb, timeoutMs) {
  if (TRANSPORT === 'ksc') return apiKsc(action, data, cb, errCb, timeoutMs);
...
function pollKscResult(seq, tries, cb, errCb) {
  // ~80s budget: first subscription fetch is capped at 30s server-side, plus
  // softcenter job dispatch / restart overhead.
  if (tries > 400) {
...
function addSub() {
  ...
  showMsg('Fetching subscription (up to 30s)…', 'info');
  // 35s > server SUB_FETCH_TIMEOUT_SECS (30) so the real error reaches the UI
...
function updateSubs() {
  showMsg('Updating subscriptions (up to 30s each)…', 'info');
...
function ensureRunning(cb) {
  api('status', null, function (d) {
    if (d && d.running) { if (cb) cb(); return; }
    showMsg('Starting Sockrocket…', 'info');
    api('start', {}, function (r) {
      toastResp(r, 'Started');
      setTimeout(function () { refreshStatus(true); if (cb) cb(); }, 1500);
    }, function () { if (cb) cb(); }, 20000);
  }, function () {
    // API down — try start via KSC/cgi anyway (start also brings up the API)
    api('start', {}, function () {
      setTimeout(function () { probeTransport(function () { refreshStatus(true); if (cb) cb(); }); }, 2000);
    }, function () { if (cb) cb(); }, 20000);
  }, 5000);
}
