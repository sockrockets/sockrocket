...
do_start() {
    ...
    # Zero-config guard: with no nodes and no subscriptions, a transparent
    # proxy would blackhole the whole LAN (and the DNS split has nothing to
    # resolve through). Start SOCKS/HTTP-only; the user adds a subscription
    # in the Web UI and restarts.
    local force_socks=0
    if [ "$(count_section nodes)" = "0" ] && [ "$(count_section subscriptions)" = "0" ]; then
        force_socks=1
        warn "No nodes or subscriptions in config -- starting in plain SOCKS/HTTP mode (transparent proxy and DNS hijack skipped)"
    ...
    if is_running; then
        ...
        if [ "$want_proxy" = "true" ]; then
            # TUN device creation happens asynchronously inside sockrocket-cli, so
            # iptables.sh would otherwise race it and bail out with "TUN device
            # not found" on a cold start — leaving the LAN unproxied until the
            # 5-minute watchdog noticed. Poll for the device instead.
        ...
        if [ "$want_dns" = "true" ]; then
            # DNS hijack is independent of the transparent proxy: the DNS
            # listener runs in every mode, so hijacking alone is valid.
            ...
            # dns_hijack is disabled but a hijack file from a previous run
            # survived — drop it, or the LAN keeps pointing at a port nobody
            # listens on.
            update_dnsmasq stop
        fi
        # Same cleanliness contract for the proxy: with the key off, a stale
        # MARK chain from a previous run (or a crash) must be torn down.
        if [ "$want_proxy" != "true" ] && \
...
do_stop() {
    ...
    # Everything below is idempotent, so running it with the daemon already
    # gone is safe.
    if is_running; then
    ...
    # A stale pid file can hide a live daemon (see do_start): stop means ALL
    # daemons die, not just the one the file happened to name.
    kill_stray_daemons
    ...
    # Record the user's stop intent: the watchdog must not resurrect the
    # service after a deliberate shutdown (crash recovery still applies when
    # this marker is absent).
    : > "$SOCKROCKET_DIR/stopped"

    ok "Sockrocket stopped"
}
...
case "$1" in
    ...
    *)
        echo "Usage: $0 {start|stop|restart|reload|status|log|diagnose|update-subs|watchdog|api-start|api-stop|api-restart|ensure-tun|dns-on|dns-off|pin-dns|proxy-off}"
        exit 1
        ;;
esac
